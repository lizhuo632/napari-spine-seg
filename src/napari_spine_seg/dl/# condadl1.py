...
class SIMDATA:
    ...
    def single_slice_info_cal(
        ...
        if "distance" not in next(iter(slice_info.values())).keys():
            if "distance" in needed_props_list:
        ...
        if (
            ...
            if "dend_touch_point" in needed_props_list:
                mask_slice = self.get_attr_by_name(mask_type + "_mask")[
                    slice_id
                ]
                spine_ids = np.unique(mask_slice)
    ...
    def spines_infos_cal(
        ...
        slices_infos = self.slices_infos_cal(
            mask_type, needed_props_list, load_exist_info, save_new
        )
        ...
        if load_exist_info and os.path.exists(spines_infos_save_path):
            ...
            spine_num = len(
                np.unique(self.get_attr_by_name(mask_type + "_mask"))
            )
            for spine_id in trange(1, spine_num):
                spine_info = dict()
                spine_info["ID"] = spine_id
                spine_info["t"] = []
                spine_info["size"] = []
                spine_info["position"] = []
                spine_info["intensity"] = []
                spine_info["puncta_num"] = []
                spine_info["puncta_size"] = []
                spine_info["dend_overlap"] = []
                spine_info["presyn_size"] = []
                spine_info["length"] = []
                spine_info["width"] = []
                spine_info["aspect_ratio"] = []
                spine_info["type"] = []
                spine_info["distance"] = []

                spines_infos.append(spine_info)

            for t in trange(
                self.get_attr_by_name(mask_type + "_mask").shape[0]
            ):
                slice_info = slices_infos[t]
                spine_ids = list(slice_info.keys())
                for spine_id in spine_ids:
                    spine_info = spines_infos[spine_id - 1]
                    spine_info["t"].append(t)
                    spine_info["size"].append(slice_info[spine_id]["size"])
                    spine_info["position"].append(
                        slice_info[spine_id]["position"]
                    )
                    spine_info["intensity"].append(
                        slice_info[spine_id]["intensity"]
                    )
                    spine_info["puncta_num"].append(
                        slice_info[spine_id]["puncta_num"]
                    )
                    spine_info["puncta_size"].append(
                        slice_info[spine_id]["puncta_size"]
                    )
                    spine_info["dend_overlap"].append(
                        slice_info[spine_id]["dend_overlap"]
                    )
                    spine_info["presyn_size"].append(
                    ...
                    spine_info["aspect_ratio"].append(
                        slice_info[spine_id]["aspect_ratio"]
                    )
                    spine_info["type"].append(slice_info[spine_id]["type"])
                    spine_info["distance"].append(
                        slice_info[spine_id]["distance"]
                    )

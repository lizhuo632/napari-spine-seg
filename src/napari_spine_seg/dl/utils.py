...
def exist_ts(mask):
    if np.max(mask) != (np.unique(mask).shape[0] - 1):
        print("max:", np.max(mask), "unique:", np.unique(mask).shape[0] - 1)
        print("reset id")
        mask = reset_id(mask)
    exist_ts = np.zeros(np.max(mask) + 1)
    for t in range(mask.shape[0]):
        exist_ts[np.unique(mask[t])] += 1
    return exist_ts
...
def meta_dice(
    ...
    sum_sizes: Tensor = (
        einsum(sum_str, [label]) + einsum(sum_str, [pred])
    ).type(torch.float32)
...
def class2one_hot(seg: Tensor, K: int) -> Tensor:
    ...
    device = seg.device
    res = torch.zeros(
        (b, K, *img_shape), dtype=torch.int32, device=device
...
def save_images(
    ...
    for seg, name in zip(segs, names):
        save_path = Path(root, f"iter{iter:03d}", mode, name).with_suffix(
            ".png"
        )
...
def augment_arr(
    *arrs_a: np.ndarray,
    ...
    if noise and random() > 0.5:
        mask: np.ndarray = np.random.laplace(
            noise_loc, noise_lambda, arrs[0].shape
        )
...
def get_center(shape: Tuple, *arrs: np.ndarray) -> List[np.ndarray]:
    ...
    def g_center(arr):
        ...
        offsets: List[int] = [
            (arrs - s) // 2 for (arrs, s) in zip(arr.shape, shape)
        ]
